...
app = FastAPI(
    title="Wycena Nieruchomości ML API (Simple)",
    description="API do wyceny nieruchomości używając modeli ML - wersja bez cache",
...
async def load_models():
    ...
    try:
        ...
        if os.path.exists(f"{models_dir}/ensemble_optimized_0.79pct.pkl"):
            ...
            logger.info(f"📊 Typ modelu: {type(ensemble_model)}")
            if isinstance(ensemble_model, dict):
                logger.info(f"📋 Dostępne modele: {list(ensemble_model.keys())}")
...
async def read_root():
    ...
    return {
        "message": "Wycena Nieruchomości ML API v2.0-simple",
        "status": "active",
        "models_loaded": {
        ...
        "cache": {
            "enabled": False,
            "message": "Cache disabled in simple version"
        }
    }
...
async def health_check():
    ...
    return {
        "status": "healthy", 
        "service": "ml-api-simple",
        "cache": {
            "status": "disabled",
            "message": "Simple version without cache"
        },
        "models": {
            "ensemble": ensemble_model is not None,
...
def get_prediction(features_df: pd.DataFrame, data: ValuationRequest):
    ...
    if ensemble_model is not None:
        try:
            logger.info(f"🔍 Próba predykcji z EstymatorAI...")
            logger.info(f"📊 Typ modelu: {type(ensemble_model)}")
            
            # EstymatorAI ensemble - używa DataFrame
            if isinstance(ensemble_model, dict):
                logger.info(f"📋 Dostępne modele w ensemble: {list(ensemble_model.keys())}")
